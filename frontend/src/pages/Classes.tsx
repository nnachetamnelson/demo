...
const ClassManagementPage = () => {
  ...
  const openAddEditModal = (cls?: ClassItem) => {
    if (cls) {
      ...
      setForm({
        ...
        formTeacherId: cls.formTeacherId?.toString() || "",
      });
    } else {
      setEditClass(null);
  ...
  // Activate / Deactivate
  const handleDeactivate = async (cls: ClassItem) => {
    try {
      await api.put(`/api/classroom/classes/${cls.id}`, { active: false });
      fetchClasses();
    } catch (err) {
  ...
  const handleActivate = async (cls: ClassItem) => {
    try {
      await api.put(`/api/classroom/classes/${cls.id}`, { active: true });
      fetchClasses();
    } catch (err) {
  ...
  const mappedClasses = classes.map((cls) => ({
    ...cls,
    level: levels.find((l) => l.id === cls.levelId)?.name || "",
    teacher: teachers.find((t) => t.id === cls.formTeacherId)?.name || "",
  }));

  const activeClasses = mappedClasses.filter((c) => c.active !== false);
  const deactivatedClasses = mappedClasses.filter((c) => c.active === false);
            ...
            <div key={cls.id} className="bg-white p-5 rounded-2xl shadow-sm relative">
              ...
              <div className="mt-5">
                <div className="border rounded-lg p-3 text-center">
                  <p className="text-2xl font-semibold">{cls.students || 0}</p>
