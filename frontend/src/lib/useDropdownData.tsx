...
const ClassManagementPage = () => {
  ...
  // Fetch dropdowns (teachers and levels)
  useEffect(() => {
  ...
  const handleSubmitClass = async () => {
    ...
    try {
      if (editClass) {
        await api.put(`/api/classroom/classes/${editClass.id}`, payload);
      } else {
        await api.post("/api/classroom/classes", payload);
      }

      setShowAddEditModal(false);
      setEditClass(null);
      setForm({ name: "", levelId: "", formTeacherId: "" });
      fetchClasses(); // refresh class list after add/edit
    } catch (err: any) {
  ...
  const handleDeactivate = async (id: number) => {
    try {
      await api.patch(`/api/classroom/classes/${id}/deactivate`);
      fetchClasses();
    } catch (err) {
      console.error(err);
    }
  ...
  const handleActivate = async (id: number) => {
    try {
      await api.patch(`/api/classroom/classes/${id}/activate`);
      fetchClasses();
    } catch (err) {
      console.error(err);
    }
